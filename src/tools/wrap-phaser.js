...
const tuneEntity = entity => {
    ...
    entity.event = key => new Promise(resolve => entity.once(key, resolve));
    entity.killTweens = () => entity.scene.tweens.killTweensOf(entity);
    return entity;
};
const tuneScene = scene => {
    scene.setTextDefaults = defaults => {
	scene.textDefaults = defaults;
	return scene;
    };
	...
	const defaultSettings = scene.textDefaults || {
    ...
    scene.input.event = key => new Promise(resolve => scene.input.once(key, resolve));
